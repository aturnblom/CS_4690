...
$(document).ready(function(){
        $.getJSON("https://lab-3-turnblom.c9users.io/api/v1/students.json", function(result){ // ../server/students.json

            
            var studentFiles = result;
            
            for (let i=0; i<STUDENT_DISPLAY_NUM; i++) {
                $.getJSON(`https://lab-3-turnblom.c9users.io/api/v1/students/${studentFiles[i]}`, function(result) {
                    students.push(result);
                    completed ++;
                   
                    
                    if (completed == STUDENT_DISPLAY_NUM) {
...
// ---------------- ADD, DELETE, UPDATE BUTTONS -----
    ...
    $("#addStudentSubmit").click(function() {
        ...
        $.ajax({
            url: 'https://lab-3-turnblom.c9users.io/api/v1/students.json',
            method: 'POST',
            data: addStudent
        });
    });
    $("#deleteStudentSubmit").click(function() {
        ...
        $.ajax({
            url: `https://lab-3-turnblom.c9users.io/api/v1/students/${deleteId}.json`,
            method: 'DELETE'
        });
    });
    $("#updateStudentSubmit").click(function() {
        ...
        $.ajax({
            url: `https://lab-3-turnblom.c9users.io/api/v1/students/${updateId}.json`,
            method: 'PUT',
            data: updateStudent
        });
    });
...
function displayTable(studentsIn){
    ...
    for (let student of studentsIn){
         studentsString.push(`<tr id=${student.id}>` + "<td>" + student.lname + ', ' + student.fname + "</td>" + "<td>" + student.startDate + "</td>" + "<td>" + student.street + "</td>" + "<td>" + student.city + "</td>" + '<td>' + student.state + "</td>" + "<td>" + student.zip + "</td>" + "<td>" + student.phone + "</td>" +  "<td>" + student.year + "</td>" + "<td>" + student.id + "</td>" +  "</tr>");
    }
    studentsString.join(' ');
...
function displayTiles(studentsIn){
    ...
    let studentsString = [];
    for (let student of studentsIn){
         studentsString.push('<div class="col-sm-4 col-md-3"><div class="panel panel-default"><div class="panel-heading">' + '<b>' + student.fname + ' ' + student.lname + '</b>' + '</div>' + '<div class="panel-body">' + 'Start: ' + student.startDate + '<br>' + 'Street: ' + student.street + '<br>' + 'City: ' + student.city + '<br>' + 'State: ' + student.state + '<br>' + 'Zip: ' + student.zip + '<br>' + 'Phone: ' + student.phone + '<br>' + 'Year: ' + student.year + '</div></div></div>');
    }
    studentsString.join(' ');
...
function yearReplacer(name, value){
    ...
    if (typeof value === 'number'){
        if (value === 1) return 'Freshman';
        else if (value === 2) return 'Sophomore';
        else if (value === 3) return 'Junior';
        else if (value === 4) return 'Senior';
    }
    return value;
}